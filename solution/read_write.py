...
def send_own_distance(particle):
    if particle.own_dist != math.inf:
        ...
        for direction in direction_list:
            neighbor_p = particle.get_particle_in(direction)
            if neighbor_p:
                if debug and debug_write:
                    print("P", particle.number, "sends own distance package", dist_package.particle_distance,
                          " to", neighbor_p.number, " in direction", direction_number_to_string(direction))
                # invert the direction so the receiver particle knows from where direction it got the package
                particle.write_to_with(neighbor_p, key=get_the_invert(direction), data=deepcopy(dist_package))
                for free_location_direction in [direction_in_range(direction - 1), direction_in_range(direction + 1)]:
                    if particle.nh_dist_list[free_location_direction].type == "fl":
                        free_location_package = message.OwnDistance(particle.nh_dist_list[free_location_direction].dist, None)
                        particle.write_to_with(neighbor_p, key=get_the_invert(free_location_direction), data=deepcopy(free_location_package))
        return True
    return False
            ...
            if neighbor_p:
                if debug and debug_write:
                    print("P", particle.number, "sends Pmax package", dist_package.p_max_dist, " to", neighbor_p.number,
                          " in direction", direction_number_to_string(direction))

                if (particle.own_dist == particle.nh_dist_list[direction].dist and send_p_max_counter == 0) or \
                        direction in particle.p_max.directions or particle.own_dist < particle.nh_dist_list[direction].dist:
                    dist_package = message.OwnDistance(particle.own_dist, particle.number)
                elif particle.own_dist == particle.nh_dist_list[direction].dist:
...
def send_to(rcver, dir, snd_buf, sender):
    if rcver and snd_buf:
        if debug and debug_write:
            print("sends ", snd_buf,
                  "to P", rcver.number, " in direction", direction_number_to_string(dir))
        # invert the direction so the receiver particle knows from where direction it got the package
