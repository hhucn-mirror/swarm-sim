...
def compare_pheromones(pheromones_dict):
    if (len(pheromones_dict) != 0):
        test = {key for key, value in pheromones_dict.items() if value[1] == 9 and min(pheromones_dict, key=pheromones_dict.get)}
        print(test)
        #return test
        return min(pheromones_dict, key=pheromones_dict.get)
    else:
...
def compare_pheromones(pheromones_dict):
    if (len(pheromones_dict) != 0):
        ...
        directions = [direction for direction, value in pheromones_dict.items() if value[1] == 9]
        if len(directions) == 0:
