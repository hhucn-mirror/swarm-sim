...
def def_p_max(particle):
    if debug and debug_p_max_calculation:
        ...
        for direction in direction_list:
            print(direction_number_to_string(direction), "|", particle.nh_dist_list[direction])
        print("Before P MAX:")
...
def find_p_max(particle):
    local_p_max(particle.own_dist, particle.p_max, particle.number, particle.nh_dist_list, particle.prev_direction, particle.p_max_table)
...
def global_p_max(p_max, rcv_buf, nh_list, own_dist):
    for rcv_direction in rcv_buf:
        if isinstance(rcv_buf[rcv_direction], message.PMax):
            if rcv_buf[rcv_direction].p_max_dist > p_max.dist:
                p_max.id = rcv_buf[rcv_direction].p_max_id
                p_max.dist = rcv_buf[rcv_direction].p_max_dist
                if nh_list[rcv_direction].dist == own_dist:
                    p_max.directions.append(rcv_direction)
                else:
                    p_max.directions = []
                p_max.black_list.clear()
                p_max.black_list.append(rcv_direction)
            elif rcv_buf[rcv_direction].p_max_dist == p_max.dist and nh_list[rcv_direction].dist == own_dist:
                p_max.directions.append(rcv_direction)
    return False
...
def local_p_max(own_distance, p_max, particle_number, nh_list, prev_direction, p_max_table):
...
def find_identical(p_max, rcv_buf):
    for rcv_direction in rcv_buf:
        if isinstance(rcv_buf[rcv_direction], message.PMax) and rcv_buf[rcv_direction].p_max_dist == p_max.dist:
            if p_max.id != rcv_buf[rcv_direction].p_max_id:
                p_max.black_list.append(rcv_direction)
...
def check_for_update(p_max, rcv_buf, particle_distance, particle_id, p_max_table):
    ...
    for rcv_direction in rcv_buf:
        if isinstance(rcv_buf[rcv_direction], message.PMax) and rcv_buf[rcv_direction].p_max_dist < p_max.dist \
                and rcv_buf[rcv_direction].p_max_id == p_max.id:
            p_max.id = rcv_buf[rcv_direction].p_max_id
            p_max.dist = rcv_buf[rcv_direction].p_max_dist
            p_max.directions.append(rcv_direction)
            p_max.black_list.clear()
            p_max.black_list.append(rcv_direction)
                ...
                and rcv_buf[rcv_direction].particle_id == p_max.id:
            p_max.id = rcv_buf[rcv_direction].particle_id
            p_max.dist = rcv_buf[rcv_direction].particle_distance
            p_max.directions.append(rcv_direction)
            p_max.black_list.clear()
            p_max.black_list.append(rcv_direction)
        elif particle_id == p_max.id and particle_distance < p_max.dist:
            p_max.id = particle_id
